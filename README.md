...
- **2.6.0** — **The description-cap gate was resolving another plugin's copy of itself.** The fallback under an unset `CLAUDE_PLUGIN_ROOT` was `find "$HOME/.claude" -name check_skill_descriptions.py -print -quit` — unscoped, and `-print -quit` takes whatever traversal order reaches first. This gate is vendored into several other plugins, so on one machine it resolved to `agent-review-panel/roundtable/3.8.2`'s copy: **19 copies present at 8 distinct hashes**. A gate that silently runs a different plugin's fork can pass a description this plugin's own version would fail, and the failure is invisible — you get an exit code, not a note about which file produced it. The lookup is now scoped to `*/publish-skill/*/scripts/…`, checks all three install roots in order, and ranks candidates on the **version segment** (`NF-2`) rather than the whole path, since the marketplace name precedes the version and a plain `sort -V` would let `aaa-mkt/2.5.0` lose to `zzz-mkt/1.0.0`. Not-found now names the three roots it tried instead of failing silently. Verified under bash, zsh and sh with `CLAUDE_PLUGIN_ROOT` unset: all three resolve this plugin's own 2.5.0 copy. Same bug class as [`session-handoff#13`](https://github.com/wan-huiyan/session-handoff/pull/13).
| Version | Date | Changes |
|---|---|---|
| 2.5.0 | 2026-08-06 | Three additions from fixing a live skill whose every check reported green while it was wrong. **New "Guards: if you cannot say how it goes red, it did not pass"** — names the rule this skill already stated three times in three specific places (the test guard that skips instead of failing, the empty `DROPPED` table blind to backticked triggers, the substring grep that cannot tell a vendored copy from a stale fork), adds the negative-control recipe, and tabulates the three shapes: a pattern that never matches, a guard that skips on a missing prerequisite, a check of presence rather than identity. A sweep of four files for a known-bad line reported all four clean — the pattern held `$` and `{`, matched nothing anywhere, and two were genuinely stale. **New "Published code is a claim"** — Step 5c now generates `tests/docs-executable.test.mjs` (every fenced block must parse; `bash -n`, not `sh -n`, since macOS ships bash 3.2), and where a doc publishes a table of inputs and outputs, run it: executing one published truth table found a real case-sensitivity bug that review had missed. **New "Releases drift because they are hand-cut"** — Step 5c now generates `release.yml` + `VERSION`, and `manifest-consistency.test.mjs` asserts the README version history lists the shipping version. Measured the same day: `claude-statusline` shipped 1.2.0 against a v1.0.0 release, and this repo sat at 2.4.2 against v2.3.0 |
| 2.4.2 | 2026-08-05 | Re-vendor the gate at `context-police@c413fd4` (**v2.3.0**), which closes two gaps in the gate's own guarantees. **Wrap corruption is now scored over every skill, disabled included, and fails the build** — it was scoped to the model-invocable subset, so a hyphen break inside a `disable-model-invocation: true` skill was neither printed nor failed. **New `NO HEADROOM` tier** (under 40 chars to spare) separates the urgent cases from `APPROACHING CAP`, which lumped everything above 1,152 chars into one bucket; this skill's description sits at **1,503 chars — 33 to spare** and now surfaces there. The pinned parity digest moves to `f72dcfab…`. Cap arithmetic unchanged, so the **850** above still stands |
